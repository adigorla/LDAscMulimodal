...
combMAT <- Matrix::readMM(paste0('./scBI_mousebrain_GEXATAC.binarized','.mtx.gz'))*1
head(combMAT)
gene.names = read.delim(paste0('./scBI_mousebrain_GEXATAC.quantitative.features','.tsv'),
                        header = TRUE,
                        stringsAsFactors = FALSE)
annots = read.delim(paste0('./scBI_mousebrain_GEXATAC.quantitative.annot','.tsv'),
                    header = TRUE,
                    stringsAsFactors = FALSE)
colnames(combMAT) <- gene.names$featID
rownames(combMAT) <- annots$cellID
...
pdf("./cplxHeatmap.pdf", width = 16, height = 8)
...
ggplot(data=data.frame(UMAP1 = Umap$layout[,1], UMAP2 = Umap$layout[,2], SRC = annots$SRC), 
...
ggplot(data=data.frame(UMAP1 = Umap$layout[,1], UMAP2 = Umap$layout[,2], batch = annots$batch), 
...
ggplot(data=data.frame(x = tSNE$Y[,1], y = tSNE$Y[,2], col = annots$SRC), 
       aes(x=x, y=y, color=col)) +
  geom_point() +
  guides(fill=guide_legend(ncol=10)) +
  theme(axis.text.y=element_blank(),
...
ggplot(tmp, aes(x = DC1, y = cell, colour = Batch)) +
    geom_point()+
  xlab("Diffusion component 1") + 
    ylab("Cell Type") +
    theme_classic()
