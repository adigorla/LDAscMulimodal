...
umpDF = data.frame(UMAP1 = Umap$layout[,1], UMAP2 = Umap$layout[,2], cell = annots$cellLabel)
ggplot() +
  geom_point( aes(x=UMAP1, y=UMAP2, group=cell), data=umpDF, colour = alpha("grey", 0.6)) +
  geom_point(data=subset(umpDF, cell == "CD8+ T" | cell == "CD8+ T naive" | cell == "CD14+ Mono" | cell == "Lymph prog" |
                           cell == "Naive CD20+ B" | cell == "CD16+ Mono"  ), 
...
ggplot() +
  geom_point( aes(x=UMAP1, y=UMAP2, group=cell), data=umpDF, colour = alpha("grey", 0.6)) +
  geom_point(data=subset(umpDF, cell == "CD4+ T activated" | cell == "CD4+ T naive" | cell == "CD14+ Mono" | cell == "Lymph prog" |
                           cell == "Naive CD20+ B" | cell == "CD16+ Mono"  ), 
             aes(x=UMAP1, y=UMAP2, color=cell)) +
  guides(fill=guide_legend(ncol=10)) +
  theme(axis.text.y=element_blank(),
